...
function renderHeader() {
    ...
    // 2. Select the Header Div
    const headerDiv = document.getElementById("header");
    if (!headerDiv) return; // Guard clause if header div is missing

    // 3. Check if the Current Page is the Root Page
    // If true, clear role and render basic logo-only header
...
// Adds event listeners to buttons dynamically created above
...
// 15. logoutPatient
// Removes the patient's session token and redirects to the patient dashboard/login.
window.logoutPatient = function() {
    localStorage.removeItem("userRole");
    localStorage.removeItem("token");
    // Redirecting to index or patient landing page
    window.location.href = "../index.html"; 
};

// 16. Render the Header
// Called to initialize the header rendering process when the script loads.
renderHeader();